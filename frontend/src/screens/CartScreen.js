    ...
    <Row>
              ...
              {cartItems.map((eachItem) => (
                <ListGroup.Item key={eachItem.product}>
                  <Row>
                    ...
                    <Col md={3}>
                      <Link to={`/product/${eachItem.product}`}>{eachItem.name}</Link>
                    </Col>
                    <Col md={2}>
                      ${eachItem.price}
                    </Col>
                    <Col md={2}>
                      <Form.Control as='select'
                        ...
                        onChange={(e) => dispatch(addToCart(eachItem.product, Number(e.target.value)))}>
                    ...
                    <Col md={2}>
                      <Button type='button' variant='light' onClick={() => removeFromCartHandler(eachItem.product)}>
      ...
      <Col md={4}>
        <Card>
          <ListGroup variant='flush'>
            <ListGroup.Item>
              <h2>Subtotal ({cartItems.reduce((acc, eachItem) => acc + eachItem.quantity, 0)}) items</h2>
              ${cartItems.reduce((acc, eachItem) => acc + eachItem.quantity * eachItem.price, 0).toFixed(2)}
            </ListGroup.Item>
          </ListGroup>
          <ListGroup>
            <Button type='button'
